...
foreach ($tasksList as $task): ?>
    ...
    <div class="white task-item" data-task="<?= $task->getId() ?>">
        ...
        <div class="task-deadline center-align <?php
        if ($task->deadlineIsOver()) {
            echo "red-text";
        } else "grey-text" ?>"><?php
            if (!is_null($deadline = $task->getDatetimeDeadline())) echo $deadline->format("d/m/Y") ?></div>
        <p class="task-actor center-align">
            <?php
            if ($task->getVisibility() != Task::VISIBILITY_ALL): ?>
                <?php
                if (!is_null($user = $task->getUserDesignated())): ?>
