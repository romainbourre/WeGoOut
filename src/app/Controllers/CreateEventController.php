    ...
    class  CreateEventController extends AppController
    {
        ...
        public function createEvent(Request $request): Response
        {
            try
            {
                ...
                $createEventRequest = new CreateEventRequest($event_title, $event_circle, $event_category,
        ...
        private function getData(Request $request): ?array
        {
            ...
            )
            {
                ...
                $event_number_participants = (int)htmlspecialchars($params['create-event-participants-number'] ?? (new AppSettings())->getParticipantMinNumber());
                ...
                if (preg_match('#^([0-9]{2})([/-])([0-9]{2})\2([0-9]{4})$#', $event_date_begin, $d) && checkdate($d[3], $d[1], $d[4]))
                { // BEGIN DATE
                    if (preg_match('#([0-9]{2})(:)([0-9]{2})#', $event_hour_begin, $h) && $h[1] >= 0 && $h[1] < 24 && $h[3] >= 0 && $h[3] < 60)
                    { // BEGIN HOUR
                ...
                {
                    if (preg_match('#^([0-9]{2})([/-])([0-9]{2})\2([0-9]{4})$#', $event_date_end, $d) && checkdate($d[3], $d[1], $d[4]))
                    { // END DATE
                        if (preg_match('#([0-9]{2})(:)([0-9]{2})#', $event_hour_end, $h) && $h[1] >= 0 && $h[1] < 24 && $h[3] >= 0 && $h[3] < 60)
                        { // END HOUR
